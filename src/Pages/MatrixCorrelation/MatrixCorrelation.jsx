...
import "./MatrixCorrelation.css";
export default function MatrixCorrelation() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post("https://django-7v4p0n.chbk.run/api/matrix-correlation/", formData, {
        headers: {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = fileName ? fileName.replace(/\.[^/.]+$/, "") + ".jpg" : "downloaded_image.jpg";
      ...
      <div className="row ">
        ...
        <div className="col-12 col-lg-2">
          <div className="ms-lg-3 mt-3 mt-lg-0" style={{ paddingTop: "40px" }}>
            <Button value="Upload" variant={uploadProgress == 100 ? "primary" : "outline"} className="ms-3" onBtnClick={handleUpload}></Button>
          </div>
        </div>
        <div className="col-12 col-lg-2"></div>
      </div>
